...
- **Release.** `release.yml` runs on `v*` tags only. `verify` checks that the tag equals `v` plus `version`, installs with `--frozen-lockfile`, audits, type-checks, tests, builds, packs, smoke-tests and uploads the tarball. `publish` runs in the `npm` environment with `id-token: write`, downloads that tarball and publishes it with `--provenance` through npm Trusted Publishing; no npm token exists anywhere. It installs nothing and runs no package scripts, and neither job restores a dependency cache. Every action in both workflows is pinned to a commit SHA.
